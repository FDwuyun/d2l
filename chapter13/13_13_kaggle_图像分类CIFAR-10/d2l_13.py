...
def train_ch13(net, train_loader, valid_loader, loss, optimizer, num_epochs, num_gpus):
    ...
    timer2.stop()
    print(f"[Total {num_epochs} epochs] {timer2.sum():.1f} sec, train_loss {metric[0] / metric[2]:.3f}, train_acc {metric[1] / metric[3]:.3f}, valid_acc {valid_acc:.3f}")
    print(f"{metric[2] * num_epochs / timer.sum():.1f} examples/sec on {str(devices)}")

# 如果param_group=True，输出层中的模型参数将使用十倍的学习率
def train_fine_tuning(net, train_datasets, test_datasets, num_epochs, batch_size, num_gpus, lr, weight_decay, param_group=True):
...
def reorg_train_valid(data_dir, labels, valid_ratio):
    """将验证集从原始的训练集中拆分出来"""
    # 训练数据集中样本最少的类别中的样本数
    # 统计字典 labels 中每个值的出现次数，并返回出现次数最少的值的出现次数
    n = collections.Counter(labels.values()).most_common()[-1][1]
    # 验证集中每个类别的样本数, 下取整
    n_valid_per_label = max(1, math.floor(n * valid_ratio))

    label_count = {}  # 创建一个空字典，用于统计每个类别的样本数量
    for train_file in os.listdir(os.path.join(data_dir, 'train')):
        # 从文件名中提取标签
        label = labels[train_file.split('.')[0]]
        # 构建文件的完整路径
        fname = os.path.join(data_dir, 'train', train_file)
        # 将文件复制到 "train_valid_test/train" 目录下，并以标签命名
        copyfile(fname, os.path.join(data_dir, 'train_valid_test', 'train_valid', label))
        # 判断是否需要将文件复制到验证集
        if label not in label_count or label_count[label] < n_valid_per_label:
            # 如果该类别样本数量不足，将文件复制到 "train_valid_test/valid" 目录下，并更新计数
            copyfile(fname, os.path.join(data_dir, 'train_valid_test', 'valid', label))
            label_count[label] = label_count.get(label, 0) + 1
        else:
            # 如果该类别样本数量已达到指定数量，将文件复制到 "train_valid_test/train" 目录下
...
def reorg_test(data_dir):
    ...
    for test_file in os.listdir(os.path.join(data_dir, 'test')):
        # 将测试集中的每个文件复制到 "train_valid_test/test/unknown" 目录下
