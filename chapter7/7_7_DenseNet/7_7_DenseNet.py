...
def transition_block(input_channels, num_channels):
    return nn.Sequential(
        nn.BatchNorm2d(input_channels), nn.ReLU(),
        nn.Conv2d(input_channels, num_channels, kernel_size=1), # 减少通道数
        nn.AvgPool2d(kernel_size=2, stride=2) # 减半高和宽
    )

# blk = transition_block(23, 10)
# summary(blk, input_data = Y)
# print(blk(Y).shape)
...
def train_ch6(net, train_iter, test_iter, num_epochs, lr, device):
    ...
    animator = Animator(xlabel=f'epoch, lr={lr}, growth_rate = {growth_rate}, batch_size={batch_size}, {device}', xlim=[1, num_epochs],
...
if __name__ == '__main__':
    ...
    print(f'\ngrowth_rate = {growth_rate}, learning_rate = {lr}, num_epochs = {num_epochs}, batch_size = {batch_size}\n')
