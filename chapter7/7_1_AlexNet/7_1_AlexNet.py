...
net = nn.Sequential(
    ...
    # 减少卷积窗口，使用填充为2来使得输入与输出的高和宽一致，且增大输出通道
...
# 实例化SummaryWriter对象
log_dir = os.path.join(script_dir, "logs", f"AlexNet_{formatted_datetime}")
...
if __name__ == '__main__':
    ...
    tb_writer.add_graph(net, input_to_model = torch.zeros((1, 1, 224, 224)))
    ...
    # 将日期和时间格式化为字符串，例如：2023-09-10-14-30-00
    formatted_datetime = current_datetime.strftime("%Y-%m-%d-%H-%M-%S")
    plt.savefig(log_dir + f"/AlexNet_Train_Chart_{formatted_datetime}.png")